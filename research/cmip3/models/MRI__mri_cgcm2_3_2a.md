...
| O  | Ozone (tropo + strato) | Y | ~ unclear | ? | **Potential Table S1 error.** The PCMDI 20C3M forcing-agent list and WDC-Climate metadata both enumerate exactly four agents (GHG, sulphate direct, volcanic, solar) and conspicuously omit ozone. No mention of ozone forcing in primary documentation. Absence-of-ozone-in-list is an inference, not an explicit exclusion statement — medium confidence only. | PCMDI MRI-GCGM2.3.2.htm §V.C (four-agent list); WDC-Climate MRI_CGCM2.3.2_20C3M_1 | yukimoto_mri_2006 | **O=Y is a candidate Table S1 error.** If ozone was not prescribed, this is a ✗ conflict. Needs explicit confirmation from Yukimoto 2001 §radiation or §forcing. |
...
- **O=Y — candidate conflict (~unclear, medium confidence):** The PCMDI 20C3M forcing-agent list and WDC metadata enumerate exactly four agents (GHG, sulphate direct, volcanic, solar) and do not include ozone. If ozone was not applied as a transient forcing, O=Y is a Table S1 error. Recorded as ~ unclear pending direct confirmation from Yukimoto 2001 radiation or forcing section.
- **No other confirmed conflicts.**

### Open questions
1. **O=Y**: Does Yukimoto et al. 2001 (§radiation or §20C3M forcing) explicitly address whether ozone was prescribed as a time-varying forcing or held fixed? Confirm or refute the O=Y claim.
2. **VL=Y dataset**: Which volcanic aerosol reconstruction? (Sato 1993 vs. Ammann 2003 vs. other?)
3. **SD temporal treatment**: Was the sulphate direct effect applied using a fixed climatology or a time-varying emission/concentration field?
