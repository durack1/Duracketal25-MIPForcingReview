...
| SS | Sea salt | Y | ✓ supports | TV (interactive diagnostic) | Sea salt is a CONFIRMED interactive diagnostic scheme driven by wind speed and height above the surface; number concentration diagnosed in two size modes (Jones et al. 2001; Roberts and Jones 2004). This is time-varying (driven by model internal wind fields). Table S1 SS=Y is correct. | Martin et al. 2006 §2b (2)Aerosols + Table 3 | — | SS=Y plausible; exact temporal treatment (TV vs FX-clim) for 20C3M not confirmed from primary documentation |
| LU | Land-use change | - | ✗ conflict (confirmed) | TV | **Goldewijk (2001) land use confirmed in Run 1** — PCMDI PDF §V Run 1 verbatim: *"Land use changes following Goldewijk (2001)."* This is the HYDE dataset providing time-varying land-cover change 1700–2000. Table S1 LU=- is a confirmed error. The seeded template (from Stage 8) had LU=Y (correct); the Table S1 row LU=- is wrong. | PCMDI HadGEM1.pdf §V Run 1 (verbatim) | goldewijk_2001 | **Table S1 LU=- is a CONFIRMED error** — Goldewijk 2001 HYDE land-use dataset is explicitly listed in Run 1 (anthropogenic forcing). Same source as other CMIP3 models applying HYDE. |
...
- **MD=Y → ✗ CONFLICT** (HIGH confidence): Mineral dust absent from HadGEM1 CLASSIC aerosol scheme; added only in HadGEM2/HiGEM. Table S1 MD=Y is wrong.
- **LU=- → ✗ CONFLICT** (CONFIRMED): PCMDI PDF §V Run 1 explicitly lists *"Land use changes following Goldewijk (2001)"*. Table S1 LU=- is a confirmed error. Run 1 (anthropogenic forcings) included HYDE land-use change.
