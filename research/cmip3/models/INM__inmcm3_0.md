...
| VL | Volcanic aerosols | n/a | ✓ confirmed | TV | **Confirmed time-varying volcanic aerosols.** PCMDI PDF §V.1 verbatim: *"Volcanic aerosols were taken from pcm_volcanic_1890-2000_T42.nc"*; *"Linear interpolation in time was used to obtain instantaneous values."* Also §II.C: *"Sulfate and volcanic aerosols, no indirect effect."* — volcanic aerosols explicitly part of the model's active forcing suite. | PCMDI INM_CM3.0.pdf §V.1 + §II.C (verbatim) | volodin_elnino_2004 | Prior adversarial refutation (0-1 vote) OVERTURNED — primary source is unambiguous. VL=TV confirmed. |
| FC | Flux corrections (F/H) | n/a | ✓ confirmed (limited) | FX-clim (annual mean, regional) | **Limited regional water flux adjustment only.** PCMDI PDF §IV.E.4 verbatim: *"Annual mean flux adjustment of water in GIN, Barentz and Kara seas only (1.75 mm/day adjustment water flux from ocean to the atmosphere in this region). No heat and momentum flux adjustment."* — freshwater only, restricted to three marginal seas; NO global heat or momentum correction. | PCMDI INM_CM3.0.pdf §IV.E.4 (verbatim) | volodin_elnino_2004 | Not a global heat/freshwater flux correction; regional water-only adjustment as coupled-model stabilizer for Arctic overflow. Under Table S1 FC convention (not applicable — model not in Table S1), this would be coded ~ or W (water only). |

### Summary of 20C3M forcing set
INM-CM3.0 applied: time-varying CO2/CH4/N2O (no CFCs) + time-varying solar (PCM1_A1.nc) + time-varying sulphate direct (2-D burden, 4ARForcing) + **time-varying volcanic aerosols (pcm_volcanic_1890-2000_T42.nc)** + fixed AMIP II ozone climatology. No indirect aerosol effect. Flux adjustment: water-only, regional (GIN/Barents/Kara seas), no heat. **All second-pass items resolved.**

### Open questions
1. **CFCs excluded**: Does this mean the GHG radiative forcing is weaker than other models? The PCM1_A1.nc file may have included CFC forcing already folded into CO2-equiv, or it may genuinely be absent. (Low priority — INM not in Table S1.)
