...
- **Model family / lineage:** GISS ModelE2 with HYCOM ocean + "-CC" designation. **CRITICAL FINDING:** The "-CC" suffix does NOT reliably indicate interactive atmospheric chemistry per se — it may denote an active coupled carbon cycle (ESM-type terrestrial + ocean biogeochemistry). Atmospheric chemistry interactivity is governed by physics_version (p1=NINT/prescribed; p2=TCAD/interactive; p3=TCADI/interactive+AIE), and the physics_version submitted for the "-CC" historical realizations was NOT confirmed in this review. See Schmidt 2014 for full configuration mapping.
...
These possibilities are not mutually exclusive. The forcing implications differ substantially.
...
| O  | ~ unclear | ? | If p3 (TCADI): exc (interactive OMA, NOT Cionni 2011); if p1 (NINT): dev (Shindell 2003) |
| SD | ~ unclear | ? | If p3: exc (interactive OMA); if p1: dev (Miller 2006a/Koch 2011) |
| SI | ~ unclear | ? | TCADI (p3) adds first indirect aerosol effect |
| BC | ~ unclear | ? | If p3: exc; if p1: dev |
| OC | ~ unclear | ? | |
| MD | ~ unclear | ? | |
| SS | ~ unclear | ? | |
| LU | ~ unclear | TV | Expected Hurtt 2011; interacts with C cycle if ESM |
| SO | ~ unclear | ? | Unconfirmed; possible Lean 2000 or Wang 2005 |
...
| BC on snow/ice | ? | GISS BC-snow capability | |
...
## Open questions
1. What does "-CC" denote: interactive carbon cycle (ESM), interactive atmospheric chemistry, or both? Schmidt 2014 should clarify.
