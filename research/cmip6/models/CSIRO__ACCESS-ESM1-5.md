...
| SO | Solar irradiance | ~ | TV | Source IS CMIP6 Matthes 2017 data, but applied with a constant +4.9 W m⁻² offset to match CMIP5-tuned piControl (piControl used CMIP5 TSI of 1365.65 W m⁻², vs CMIP6 mean of 1360.75 W m⁻²). Model uses TSI only (no spectral variation). | Ziehn et al. 2020 §3.1 | "The historical and future scenario simulations used the CMIP6 data with a constant 4.9 W m⁻² offset." CMIP6 Matthes 2017 is the source dataset but constant-offset modification gives ~ rather than ✓std. |
| VL | Volcanic aerosols | ~ | TV | Source IS CMIP6 AOD data (Thomason 2018 / IACETH at 550 nm), but collapsed to 4 equal-area latitude bands (cannot use height/wavelength-resolved 3D IACETH data). Additional uniform +0.00271 AOD offset applied to match piControl (piControl uses AOD=0.01338 vs CMIP6 mean=0.01067). | Ziehn et al. 2020 §3.1 | piControl: extended Sato 1993 (CMIP5 forcing, 4-band). Historical: CMIP6 AOD source used but in simplified 4-band form. CMIP6 data IS the source but reduced-fidelity implementation gives ~ rather than ✓std. |
...
1. **Solar (SO) — ~**: CMIP6 Matthes 2017 data IS the source, but applied with a constant +4.9 W m⁻² offset to reconcile with CMIP5-era piControl tuning (piControl TSI=1365.65 vs CMIP6 mean=1360.75 W m⁻²). Model uses TSI only (not spectral variation). Standard source data, non-standard implementation = ~.
2. **Volcanic (VL) — ~**: CMIP6 AOD (Thomason 2018/IACETH) IS the source at 550 nm, but simplified to 4 latitude bands (cannot ingest height/wavelength-resolved full CMIP6 dataset); uniform +0.00271 AOD offset applied to match piControl. Standard source data, reduced-fidelity implementation = ~.
