...
| LU | Land-use change | ~ | TV *(likely)* | CoLM land model — **LU dataset not named in Zhang 2020**. FGOALS-f3-L (CAS, same CoLM land model) was confirmed ✗dev using LUH1/Hurtt 2011. UofMD-landState-2-1-h (LUH2 v2.1h) is the only CMIP6 historical land-state dataset in input4MIPs; neither LUH2 nor any land-use dataset is named in Zhang 2020. | Zhang 2020; ESGF input4MIPs catalog 2026-06-28 | **Inference from shared infrastructure**: CoLM land model also used in FGOALS-f3-L (confirmed LUH1 ✗dev). CAS-ESM2-0 LU likely also ✗dev but requires direct confirmation (Zhang 2020 or CoLM-specific paper). |
